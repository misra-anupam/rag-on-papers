...
def parse_tei(tei_xml: str) -> dict:
    ...
    return {
        ...
        'mesh_terms': [],
        'keywords':  [k.text for k in root.findall('.//tei:keywords/tei:term', namespaces=NS)
                      if k.text],
...
def _extract_sections(root) -> list[dict]:
    ...
    for div in root.findall('.//tei:body/tei:div', namespaces=NS):
        ...
        paras     = [p.text_content() if hasattr(p, 'text_content') else (p.text or '')
...
def _table_to_markdown(table_elem) -> str:
    ...
    for i, row in enumerate(rows):
        cells = [c.text_content().strip() if hasattr(c, 'text_content')
                 else (c.text or '')
