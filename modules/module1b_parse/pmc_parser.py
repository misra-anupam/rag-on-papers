...
def _extract_sections(root) -> list[dict]:
    ...
    for sec in body.findall(".//sec"):
        ...
        fig_refs = [xref.get("rid", "") for xref in sec.findall('.//xref[@ref-type="fig"]')]
        tbl_refs = [xref.get("rid", "") for xref in sec.findall('.//xref[@ref-type="table"]')]
        sections.append(
            {
                "heading": heading,
...
def _extract_figures(root) -> list[dict]:
    ...
    for fig in root.findall(".//fig"):
        fig_id = fig.get("id", "")
        caption = (
            " ".join(fig.find("caption").itertext()).strip()
...
def _extract_tables(root) -> list[dict]:
    ...
    for tbl_wrap in root.findall(".//table-wrap"):
        ...
        caption = (
            " ".join(tbl_wrap.find("caption").itertext()).strip()
            if tbl_wrap.find("caption") is not None
            else ""
        )
