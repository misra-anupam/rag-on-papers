...
def _base_fields(paper: dict) -> dict:
    authors = [a["name"] if isinstance(a, dict) else str(a) for a in paper.get("authors", [])]
...
def chunk_figure(fig: dict, paper: dict, chunk_index: int) -> Chunk | None:
    ...
    if not text:
        return None
    return _make_chunk(
        text, f"Figure {fig.get('fig_id', '')}", "figure", chunk_index, 0, paper, has_figure=True
    )
...
def chunk_table(table: dict, paper: dict, chunk_index: int) -> Chunk | None:
    ...
    if not text:
        return None
    return _make_chunk(
        text, f"Table {table.get('table_id', '')}", "table", chunk_index, 0, paper, has_table=True
    )
