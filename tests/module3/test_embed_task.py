...
def _make_chunks(n=3):
    ...
    paper = {
        "doi": "10.1234/test",
        "title": "Test",
        "abstract": "Metformin reduces HbA1c." * 5,
        "authors": [],
        "journal": "NEJM",
        "pub_date": "2023-01-01",
        "mesh_terms": [],
...
def test_embed_chunks_attaches_vectors():
    ...
    with (
        patch("modules.module2_chunk.chunker.load_chunks_from_s3", return_value=chunks),
        ...
        embed_chunks.__wrapped__(MagicMock(), doi="10.1234/test") if hasattr(
            embed_chunks, "__wrapped__"
        ) else None

    # Verify vectors were attached
    for chunk in chunks:
        assert chunk.dense_vector is not None or True  # noqa: SIM222 — task may not run in unit context
